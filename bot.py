...
def get_known_chat_ids() -> list[int]:
    """Список всех групп, о которых бот вообще что-либо знает (куда-то
    писали люди). Нужно, чтобы понимать, в какую группу добавлять человека,
    если команда /add прислана боту в личные сообщения, а не в саму группу."""
    with closing(sqlite3.connect(DB_PATH)) as conn:
        cur = conn.execute("SELECT DISTINCT chat_id FROM users")
        return [row[0] for row in cur.fetchall()]


async def resolve_target_chat_id(message: Message) -> int | None:
    """Определяет, в какую группу должна применяться команда.
    Если команда написана прямо в группе — используем её.
    Если написана боту в личку — используем единственную известную боту
    группу (для одной учебной группы этого достаточно). Если групп
    несколько или ни одной — сообщаем об этом и возвращаем None."""
    ...
    if len(chat_ids) == 0:
        await message.answer(
            "🤷 Бот пока не знает ни одной группы — он должен хотя бы раз "
            "увидеть сообщение в группе, чтобы её запомнить."
        )
        return None

    await message.answer(
        "⚠️ Бот работает больше чем с одной группой, из личных сообщений "
        "не могу понять, к какой применить команду. Напишите её прямо в "
        "нужной группе."
    )
    return None
...
async def handle_help(message: Message):
    ...
    text = (
        ...
        "Добавить человека в список упоминаний по username (собачка @ не "
...
async def handle_add_mention(message: Message):
    ...
    Собачка @ перед username необязательна. Работает как прямо в группе,
    так и в личных сообщениях боту (если бот знает только одну группу)."""
    ...
    if not usernames:
        await message.answer(
            ...
            "введённого username).\n"
            "Укажите username через пробел (собачка @ не обязательна), "
            "например:\n"
...
async def handle_list(message: Message):
    """Показывает текущий список известных боту людей в виде готовой
    команды /add — удобно скопировать и сохранить себе (например, закрепить
    сообщение в чате) на случай, если база данных снова случайно очистится.
    /add никого не пингует, так что восстановление безопасно и мгновенно.
    Работает и в группе, и в личных сообщениях боту."""
    if not await require_access(message):
        return
    ...
    if not users:
        await message.answer("🤷 База пуста, добавлять пока нечего.")
        return

    usernames = [f"@{uname}" for _, uname, _ in users if uname]
    no_username = [fname for _, uname, fname in users if not uname]

    lines = [f"📋 <b>Известно людей: {len(users)}</b>\n"]
    if usernames:
        lines.append(
            "Сохраните эту команду (например, закрепите сообщение) — если "
            "база вдруг очистится, просто отправьте её ещё раз, это не "
            "пингует людей и восстановит список мгновенно:\n\n"
            f"<code>/add {' '.join(usernames)}</code>"
        )
    if no_username:
        lines.append(
            "\n⚠️ Без username (добавятся автоматически, когда сами "
            f"напишут в чат): {', '.join(no_username)}"
        )
    await message.answer("\n".join(lines))
...
async def main():
    init_db()
    # Регистрируем команды, чтобы они показывались в меню "/":
    # отдельно для групп и отдельно для личных сообщений с ботом
    # (/add и /list теперь можно писать и туда, и туда).
